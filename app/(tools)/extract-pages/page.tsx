...
import { config } from '@/content/tools/extract-pages'
...
export default function ExtractPagesPage() {
  ...
  const handleConvert = async () => {
    ...
    const range = pageRange.trim() || '1'
    setPhase('converting')
    setProgress(0)
    try {
      const rawResults = await extractPages(
        files,
        { pageRange: range, separatePages },
  ...
  const handleDownloadAll = useCallback(async () => {
    await downloadAsZip(results, 'extracted-pages.zip')
  }, [results])
      ...
      <div className="rounded-2xl border border-border bg-bg-elevated p-6 shadow-sm">
        ...
        {phase === 'ready' && files.length > 0 && (
          <div className="space-y-5">
            ...
            <fieldset>
              <legend className="mb-1.5 text-xs font-medium text-fg">Pages to extract</legend>
              <input
                type="text"
                value={pageRange}
              ...
              <p className="mt-1 text-xs text-fg-subtle">
                Comma-separated page numbers and ranges. Example: <span className="font-mono">1, 3, 5-8</span>. Leave blank to extract page 1.
              </p>
            </fieldset>
            ...
            <label className="flex cursor-pointer items-center gap-2.5">
              ...
              />
              <span className="text-sm text-fg">One PDF per page</span>
              <span className="text-xs text-fg-subtle">(instead of combining all selected pages)</span>
            </label>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={handleConvert}
                ...
                Extract pages from {files.length} PDF{files.length > 1 ? 's' : ''}
              </button>
            </div>
          </div>
        )}
        ...
        {phase === 'converting' && (
          <div className="space-y-4 py-4 text-center">
            <p className="text-sm font-medium text-fg">Extracting pages…</p>
        ...
        {phase === 'error' && (
          <div className="space-y-5">
            <div className="flex items-start gap-3 rounded-xl border border-error/40 bg-bg px-4 py-3">
              ...
              <div>
                <p className="text-sm font-medium text-fg">Extraction failed</p>
